...
- **Live version: v3.36.0** (on `main`). **v3.35.0 = phased adaptive onboarding** (tutorial). **v3.36.0 = combat perf + balance cuts** (below). **⚑ Super Events engine + Event #1 are CODE-COMPLETE but HELD DORMANT** (`EVENTS_LIVE=false` near the EVENTS array) — the designer wants a full BATCH of events before the decision layer goes public. Flip the flag + restore the "v3.35.0 — THE FIRST DECISION" changelog para to ship it.
...
- **"Superheavy Aircraft"** research (pre: Fusion/Space Age + shielding) → **Sky Destroyers & Sky Battleships**.
...
- **DECISIONS LOCKED:** (1) capital ships are a NEW TIER ABOVE the airship/carrier, not replacements (you climb to them); (2) Miniaturized Shielding = shared root; (3) they ARE carriers.
