...
This is an **evolution of the existing dismissable guidance panel**, not a new system: the panel's *fixed* suggested path becomes a *dynamic* path generated from player-selected goals. It stays loose/non-gating; the only scripted stretch is the intro framing (beats 1–2), which should be light and skippable on replay. NOTE: the depth of the reverse-planned steps scales with how real the supply chain is — the shallow "build lab → research → build factory" of today deepens as the atlas's titanium/electronics chains get built. The tutorial's richness and the supply chain's realness are the same project.
