...
- **AWAITING DESIGNER:** (a) **CIWS-perf + auto-emplacement nerf + THIN/TINY tracers** (make emplacement-gun tracer projectiles thin & small like real tracers) — one lane, still wanted (live lag + unstoppable free wall), re-fire on go. (b) **MORALE/ORGANIZATION** — unified plan now banked (roster susceptibility + layers); ready to build Layer-1 spine on go. (c) Enemy-escalation v2. Lethality = REVERSED, do not build.
