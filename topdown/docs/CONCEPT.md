...
**Interactive in-game tech/production tree (direction):** an intuitive, browseable, pannable node-graph VIEW of the production + research trees inside the game — distinct from the static external atlas doc. It's what the tutorial's goal-selection reads from and what "fills out" as you build. Lineage: v1's research tree was already a big pannable canvas. This is the heaviest single piece of the onboarding concept and the home of the "we're making jets now" payoff.
